...
An LLM-backed planner that turns the natural-language input request into a constraint-graph scene JSON (points + relations + label slots), with retry feedback on `UnsatisfiableConstraintsError` (which also surfaces infeasibility — the planner / solver loop is how unrenderable requests are detected). Derives from legacy capability `svg.scene-to-svg` (legacy planner pipeline), reshaped per the proposal. Required because the additional guidance locks the LLM as used ONLY for "natural-language description → constraint-graph JSON", making this the single permitted LLM role and the sole source of the scene model that the deterministic render pipeline consumes.
